...
async def login(update, ctx):
    if update.effective_user.id != OWNER_ID or len(ctx.args) < 2:
        return
    ...
    try:
        ...
        accounts[name] = {"ctx": context, "page": page}
...
async def spam(update, ctx):
    ...
    if update.effective_user.id != OWNER_ID or not messages or not target or not accounts:
        return
